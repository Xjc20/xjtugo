...
class my_player():
    ...
    def get_opponent_pieces(self, go: GO, piece_type: int) -> list:
        """
        获取棋盘上对手已落子的所有坐标。
        :param go: GO 实例
        :param piece_type: 我方棋子类型（1 或 2）
        :return: 包含对手棋子坐标的列表 [(x1, y1), (x2, y2), ...]
        """
        opponent_pieces = []
        opponent_type = 3 - piece_type
        for x in range(go.size):
            for y in range(go.size):
                if go.board[x][y] == opponent_type:
    ...
    def calculate_score(self, go:GO, piece_type):
        ...
        opponent_pieces = self.get_opponent_pieces(go, piece_type)
    ...
    def cached_minimax(self, piece_type, go, depth, alpha, beta, is_maximizing):
        ...
        if is_maximizing: #最大最小化
            ...
            for move in self.get_possible_placements(go, piece_type):
                x, y = move  # 将元组拆分为两个整数
                saved_previous_board = deepcopy(go.previous_board)
                ...
                go.previous_board = saved_previous_board
