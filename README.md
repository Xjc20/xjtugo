...
├── play_vs_with_host.bat # Windows系统构建脚本
...
# 示例：AlphaZero AI vs 随机玩家（命令行模式）
play_vs_with_host.bat 4 my_player alpha/players/random_player host

# 示例：人类玩家 vs AlphaZero AI（GUI模式）
play_vs_with_host.bat 4 human_gui_player my_player host_gui
```
...
### 训练数据统计
- 训练数据保存在 `alpha/training_data/` 目录下
- 文件名格式：`data_YYYYMMDD_HHMMSS.pkl`
- 总数据量：约15万局自我对弈数据
- 单文件大小：约5-10MB
