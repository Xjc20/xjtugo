"""
文件功能: 围棋游戏主机程序，负责管理游戏状态、规则判断和玩家交互

主要功能:
1. GO类: 实现围棋游戏的核心逻辑，包括棋盘管理、落子规则、提子规则、禁着点(KO)规则
2. 游戏状态管理: 初始化棋盘、更新棋盘、判断游戏结束
3. 胜负判定: 根据棋子数量和贴目规则判定胜负
4. 玩家交互: 支持人机对战和AI对战

参数说明:
- n: 棋盘大小 (默认5x5)
- piece_type: 棋子类型 (1=黑子X, 2=白子O)
- board: 当前棋盘状态 (二维列表，0=空, 1=黑子, 2=白子)
- previous_board: 上一步棋盘状态 (用于KO规则判断)
- komi: 贴目 (默认n/2)

使用方法:
1. 作为模块导入: from host import GO
2. 命令行运行: python host.py -m <移动次数> -v <是否显示棋盘>
   示例: python host.py -m 10 -v True
"""
