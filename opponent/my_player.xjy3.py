...
def apply_move(board, prev_board, move, piece):
    """
    在 board 上落子，返回:
    (是否合法, 新棋盘, 新的 previous_board)
    """
    if move is None:
        return True, fast_copy(board), fast_copy(board)
    ...
    # 先提掉对手无气块
    remove_dead_groups(new_board, other(piece))
    ...
    return True, new_board, fast_copy(board)
...
class MCTS:
    ...
    def legal_moves(self, board, prev_board, piece):
        ...
        for i in range(BOARD_SIZE):
            for j in range(BOARD_SIZE):
                if board[i][j] != EMPTY:
                    continue
                ok, _, _ = apply_move(board, prev_board, (i, j), piece)
                if ok:
                    moves.append((i, j))
    ...
    def move_priority(self, board, prev_board, move, piece):
        ...
        for dx, dy in DIRS:
            ...
            if board[nr][ny] == opp and (nr, ny) not in seen_opp:
                ...
                if (r, c) in liberties:
                    if len(liberties) == 1:
                        score += 5.0 + 1.8 * len(group)
                    elif len(liberties) == 2:
                        score += 1.5 + 0.5 * len(group)
            ...
            # 补棋救己方打吃块
                ...
                if (r, c) in liberties and len(liberties) == 1:
                    score += 3.0 + 0.8 * len(group)

        ok, new_board, _ = apply_move(board, prev_board, move, piece)
        if not ok:
            return -1e9
    ...
    def best_child(self, node):
        ...
        c = 1.25 if node.visits < 80 else 0.85
        ...
        for move, child in node.children.items():
            ...
            prior = self.move_priority(node.board, node.prev_board, move, node.player) * 0.03
    ...
    def select_and_expand(self, root):
        ...
        while True:
            ...
            if untried:
                move = untried[0]
                ok, new_board, new_prev = apply_move(board, prev_board, move, player)
                if not ok:
                    return node, board, prev_board, player
    ...
    def rollout(self, board, prev_board, player, root_piece):
        ...
        for _ in range(self.rollout_depth):
            ...
            ranked = moves[:min(4, len(moves))]
            move = ranked[0]
            if len(ranked) >= 2 and self.random.random() < 0.35:
                move = self.random.choice(ranked[:2])
            ...
            ok, new_board, new_prev = apply_move(sim_board, sim_prev, move, turn)
            if not ok:
                consecutive_pass += 1
                if consecutive_pass >= 2:
                    break
    ...
    def opening_move(self, board, prev_board, piece):
        ...
        for move in candidates:
            ok, _, _ = apply_move(board, prev_board, move, piece)
            if not ok:
                continue
